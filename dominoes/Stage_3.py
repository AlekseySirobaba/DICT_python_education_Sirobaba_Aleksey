...
class Domino:
    ...
    def players_input(self):
        while True:
            ...
            if (dom_pos.isdigit() or '-' in dom_pos and dom_pos.strip('-')) \
                ...
                if dom_pos < 0:
                    self.snake.insert(0, self.player[abs(dom_pos) - 1])
                    self.player.remove(self.player[abs(dom_pos) - 1])
                    break
    ...
    def comp_input(self):
        ...
        # If number is negative, then domino add to left side
        if dom_pos < 0:
            self.snake.insert(0, self.computer[abs(dom_pos) - 1])
            self.computer.remove(self.computer[abs(dom_pos) - 1])
